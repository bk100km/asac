								...
								<table id="orderListTable" class="table table-striped table-bordered table-hover">
									...
									<c:forEach var="order" items="${orderSellerList}">
										<tr id="orderInfoBtn" data-ocode="${order.ocode}" data-pname="${order.pname}" data-odelivery="${order.odelivery}" onclick="orderInfoAction(this)">
											...
											<td>${order.pname}</td>
											<td>${order.ocount}</td>
											<td><input type="hidden" name="odeliverycheck" id="odeliverycheck" value="${order.odelivery}"/>
												<select name="delivery" id="selectDeliveryInfo" class="form-control custom-select" required="required" onchange="deliveryUpdate(this.value);">

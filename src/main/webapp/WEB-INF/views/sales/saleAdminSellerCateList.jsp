...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${adSellerCateList}
		var jsonDataConfirm = ${adSellerCateConfirmList}
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleAdminSellerCateList}" varStatus="status">
						<tr class="line">
							<td><a href="./ap/pcate/${sale.pcate}">${sale.pcate}</a></td>
							<td>
								<c:choose>
								<c:when test="${saleAdminCateProductConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleAdminCateProductConfirmList[status.index].count ne null}">${saleAdminCateProductConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminCateProductConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleAdminCateProductConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleAdminCateProductConfirmList[status.index].total}" pattern="#,###"/>원</c:when>
								</c:choose></td>

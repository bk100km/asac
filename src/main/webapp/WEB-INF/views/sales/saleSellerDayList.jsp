				...
				<tbody>
				  <c:forEach var="sale" items="${saleSellerDayList}" varStatus="status">
						<tr class="line">
							...
							<td>
								<c:choose>
								<c:when test="${saleSellerDayConfirmList[status.index].count eq 0}">0</c:when>
								<c:when test="${saleSellerDayConfirmList[status.index].count ne 0}">${saleSellerDayConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>

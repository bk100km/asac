...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${selweekList}
		var jsonDataConfirm = ${selweekConfirmList}
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleSellerWeekList}" varStatus="status">
						<tr class="line">
							<td>${sale.week}</td>
							<td>
								<c:choose>
								<c:when test="${saleSellerWeekConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleSellerWeekConfirmList[status.index].count ne null}">${saleSellerDayConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>
								<c:when test="${saleSellerWeekConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleSellerWeekConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleSellerDayConfirmList[status.index].total}" pattern="#,###"/>원</c:when>
								</c:choose></td>

...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${adweekList}
		var jsonDataConfirm = ${adweekConfirmList}
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleAdminWeekList}" varStatus="status">
						<tr class="line">
							<td>${sale.week}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminWeekConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleAdminWeekConfirmList[status.index].count ne null}">${saleAdminWeekConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminWeekConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleAdminWeekConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleAdminWeekConfirmList[status.index].total}" pattern="#,###"/>원</c:when>
								</c:choose></td>

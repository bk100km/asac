...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${addateList}
		var jsonDataConfirm = ${addateConfirmList}
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleAdminDateList}" varStatus="status">
						<tr class="line">
							<td>${sale.date}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminDateConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleAdminDateConfirmList[status.index].count ne null}">${saleAdminDateConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminDateConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleAdminDateConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleAdminDateConfirmList[status.index].total}" pattern="#,###"/>원</c:when>
								</c:choose></td>

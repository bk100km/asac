...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${addayList}
		var jsonDataConfirm = ${addayConfirmList}
	...
	<div id="rListArea">
		<div class="table-responsive">
			<table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleAdminDayList}" varStatus="status">
						<tr>
							<td class="show">${sale.daterange}</td>
							<td class="hide">
								<c:choose>
								<c:when test="${saleAdminDayConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleAdminDayConfirmList[status.index].count ne null}">${saleAdminDayConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td class="hide">${sale.count}</td>
							<td class="show"><c:choose>
								<c:when test="${saleAdminDayConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleAdminDayConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleAdminDayConfirmList[status.index].total}" pattern="#,###,###"/>원</c:when>
								</c:choose></td>

...
<script type="text/javascript">
	...
	function getGraph(){
		var jsonData = ${adcateProductList}
		var jsonDataConfirm = ${adcateProductConfirmList}
...
<div class="table-title">
	...
	<h1 class="mainTitle"> <%= category %> 매출 상세정보</h1></div>
<div>
    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            ...
            <table class="table table-striped table-hover">
				...
				<tbody>
				  <c:forEach var="sale" items="${saleAdminSellerCateProductList}" varStatus="status">
						<tr class="line">
							<td>${sale.pname}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminSellerCateProductConfirmList[status.index].count eq null}">0</c:when>
								<c:when test="${saleAdminSellerCateProductConfirmList[status.index].count ne null}">${saleAdminSellerCateProductConfirmList[status.index].count}</c:when>
								</c:choose></td>
							<td>${sale.count}</td>
							<td>
								<c:choose>
								<c:when test="${saleAdminSellerCateProductConfirmList[status.index].total eq null}">0원</c:when>
								<c:when test="${saleAdminSellerCateProductConfirmList[status.index].total ne null}"><fmt:formatNumber value="${saleAdminSellerCateProductConfirmList[status.index].total}" pattern="#,###"/>원</c:when>
								</c:choose></td>
